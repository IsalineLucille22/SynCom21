...
data_to_save_SynCom21_Reduced = struct(); %To avoid overwriting, change the name of the save variable at the last line of the script. Name it accoridng to this structure name.
...
addpath('/Users/iguex/Library/CloudStorage/OneDrive-UniversitédeLausanne/CoCulture_Soil/21 SynCom Script (with Lysobacter)/Data')
...
for nb_iter_tot = 1:1
    ...
    n_fake = 3;
    Fake_Data = zeros(S, nb_time_step, n_fake);
    for i = 1:S
        for t = 1:nb_time_step
            mu = mean(Measured_Abund(i,t,:), 3);
            sigma = std(Measured_Abund(i,t,:), 1, 3);
            if sigma == 0
                sigma = mu*0.05; % small relative noise if no replicates
            end
            Fake_Data(i,t,:) = max(normrnd(mu, sigma, [1,1,n_fake]), 0);
        end
    end
    Measured_Abund = cat(3, Measured_Abund, Fake_Data);
    StackPlot_Meas = Measured_Abund./sum(Measured_Abund);
    ...
    t_0 = 0; %Time 0
    nb_Res = 12;%12; %Number of resources (group of resources)
    nb_Res_max = 12;%12;
    ...
    beta = 0.5*1e2; 
    num_steps = 100;%500;
    nb_loops = 3;%4; %Number of different loops (CF, death, resources, predation), 3 if predation loop is commented
    ...
    Threshold_CF_init = Threshold_CF;
    ...
    while p < N
        ...
        disp(sum(accept_temp)/(num_steps))
        burn_in = round(0.3*size(param_chain_3, 2));
        param_chain_post = param_chain_3(:, :, burn_in:h);
        ...
        disp(sum(accept_temp)/(num_steps))
        Cov_Mat_Res = Cov_Mat;
        burn_in = round(0.3*size(param_chain_4, 2));
        param_chain_post = param_chain_4(:, :, burn_in:h);
        
        % Compute mean, std, and CV for each parameter
        param_mean = mean(param_chain_post, 2);
        param_std  = std(param_chain_post, 0, 2);
        CV_4 = param_std./abs(param_mean);
        %%%
    
        Resource_Matrix_cand = Resource_Matrix; nb_Res_new = nb_Res; R_new = R; mat_y_0_new = mat_y_0; %Re-initialization if the last candidate is not accepted. Otherwise issues with the resource loop.
    ...
    data_to_save_SynCom21_Reduced(nb_iter_tot).CrossFeed_Mat_Temp = CrossFeed_Mat_Temp;   
    data_to_save_SynCom21_Reduced(nb_iter_tot).Pred_Mat_Lyso = Pred_Mat_Lyso;
    data_to_save_SynCom21_Reduced(nb_iter_tot).Threshold_Pred = Threshold_Pred;
    data_to_save_SynCom21_Reduced(nb_iter_tot).Threshold_CF = Threshold_CF;
    data_to_save_SynCom21_Reduced(nb_iter_tot).Threshold_death = Threshold_death;
    data_to_save_SynCom21_Reduced(nb_iter_tot).Death_Mat_Temp = Death_Mat_Temp;
    data_to_save_SynCom21_Reduced(nb_iter_tot).Resource_Matrix = Resource_Matrix;
    data_to_save_SynCom21_Reduced(nb_iter_tot).Lag_time_Pred = Lag_time_Pred;
    data_to_save_SynCom21_Reduced(nb_iter_tot).Lag_time_Cons = Lag_time_Cons;
    data_to_save_SynCom21_Reduced(nb_iter_tot).Mat_kappa_3 = Mat_kappa_3;
    data_to_save_SynCom21_Reduced(nb_iter_tot).kappa_mat = kappa_mat;
    data_to_save_SynCom21_Reduced(nb_iter_tot).R = R;
    data_to_save_SynCom21_Reduced(nb_iter_tot).CrossFeed_Mat_init = CrossFeed_Mat_init;
    data_to_save_SynCom21_Reduced(nb_iter_tot).Threshold_CF_init = Threshold_CF_init;
    data_to_save_SynCom21_Reduced(nb_iter_tot).Pred_Mat_Lyso_init = Pred_Mat_Lyso_init;
    data_to_save_SynCom21_Reduced(nb_iter_tot).Threshold_death_init = Threshold_death_init;
    data_to_save_SynCom21_Reduced(nb_iter_tot).Death_Mat_init = Death_Mat_init;
    data_to_save_SynCom21_Reduced(nb_iter_tot).Resource_Matrix_init = Resource_Matrix_init;
    data_to_save_SynCom21_Reduced(nb_iter_tot).Lag_time_Pred_init = Lag_time_Pred_init;
    data_to_save_SynCom21_Reduced(nb_iter_tot).Lag_time_Cons_init = Lag_time_Cons_init;
    data_to_save_SynCom21_Reduced(nb_iter_tot).Mat_kappa_3_init = Mat_kappa_3_init;
    data_to_save_SynCom21_Reduced(nb_iter_tot).kappa_mat_init = kappa_mat_init;
    data_to_save_SynCom21_Reduced(nb_iter_tot).R_init = R_init;
    data_to_save_SynCom21_Reduced(nb_iter_tot).Struct_Res_Mat = Struct_Res_Mat;
    data_to_save_SynCom21_Reduced(nb_iter_tot).Struct_Death_Mat = Struct_Death_Mat;
    data_to_save_SynCom21_Reduced(nb_iter_tot).Struct_CrossFeed_Mat = Struct_CrossFeed_Mat;
    data_to_save_SynCom21_Reduced(nb_iter_tot).Struct_Pred_Mat_Lyso = Struct_Pred_Mat_Lyso;
    data_to_save_SynCom21_Reduced(nb_iter_tot).Cov_Mat_CF = Cov_Mat_CF;
    data_to_save_SynCom21_Reduced(nb_iter_tot).Cov_Mat_Death = Cov_Mat_Death;
    data_to_save_SynCom21_Reduced(nb_iter_tot).Cov_Mat_Res = Cov_Mat_Res;
    data_to_save_SynCom21_Reduced(nb_iter_tot).death_rate = death_rate;
end

save(strcat(cd, '/Data/', 'data_to_save_SynCom21_Reduced'), 'data_to_save_SynCom21_Reduced')