...
# -*- coding: utf-8 -*-
"""
Created on Tue Nov  4 14:46:20 2025
...
# --- File paths
# -------------------------------------------------------------------
path = "/Users/iguex/Library/CloudStorage/OneDrive-UniversitédeLausanne/CoCulture_Soil/21 SynCom Script (with Lysobacter)/Reduced Fitting/Data/"
name_file = path + "Monoculture_soil_nonstrictcellgate_SenkaCB.xlsx"
...
# --- Load data
# -------------------------------------------------------------------
data = pd.read_excel(name_file, "Sheet4")
nb_species = np.shape(data)[0]
Names = data.time_days
time_init = data.columns[1:]
time_step_tot = list(map(remove_last, time_init))
time_step = np.array(time_step_tot, dtype=np.float64).reshape(-1, 1)
...
with pd.ExcelWriter("Data/Mono_Init_abund.xlsx") as writer:
