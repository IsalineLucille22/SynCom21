function dz_vect = fun_CF_Death_Lyso(t, z, kappa, CrossFeed_Mat, Mat_kappa_3, Resource_Matrix, Threshold_CF, Threshold_death, Threshold_Pred, Death_Mat, death_rate, Pred_Mat, yield_Pred, S, Lag_time_Cons, Lag_time_Pred, nb_Res, Hill_CF, Hill_Pred, y_0)
z(z<0) = 0; %
...
T(isnan(T)) = 0;
k = Hill_Pred;%10;
T_death = (1./(1 + ((y_0 + Threshold_death(1))./z(:,1)).^k));%(1./(1 + (Threshold_death(1)./(max(z(:,3) - sum(r),0))).^k));%Threshold on resource concentration?
...
T_pred = 0;%(1./(1 + (Threshold_Pred(1)./z(6,1)).^5)).*(1./(1 + (1e-05./z(:,1)).^5));%0 when Monoculture
...
dy_vect = zeros(S,1); %z(:,1).*sum(CrossFeed_Mat.*((T.*z(:,3))'./(z(:,3)' + K_S_Waste)),2);%z(:,1).*sum(CrossFeed_Mat.*((T.*z(:,3))'./(z(:,3)' + K_S_Waste)),2);%Byproduct growth, just for information
...
dz_vect = [dz_vect dr]';
if sum(z(:,1).*((t > Lag_time_Pred).*Pred_Mat).*(T_pred.*z(:,1))'./(z(:,1)' + K_S_Pred))'> 0
    c = 10;
end
end