...
name_Exp = 'Bruna_21_CF_Reduce_Res';%'Bruna_21_CF';
...
%possibilities
max_obs_biomass = mean(table2array(Data_Evol(:,32:36)),2); %mean(table2array(Data_Evol(:,27:31)),2); %mean(table2array(Data_Evol(:,14:17)),2); %mean(table2array(Data_Evol(:, 22:25)),2); %timepoint 21 %timepoint 7 %
max_obs_biomass = [table(name),array2table(max_obs_biomass), Data_Evol(:,14:17)];%[table(name),array2table(max_obs_biomass), Data_Evol(:, 22:25)];%
...
%Automatized this
for i = 3:6
    scatter(flip(log10(table2array(max_obs_biomass(:,i)./(single_cell_mass.assumedCellDwInFgC*1e-15)))),1:S,50,'.','b')
end
...
%% Comparison individual Observed vs simulated. We simulate the biomass after one week and not 21 days
close all 
...
%We compare the time point 21 days of the SynCom20 to time point 22 days of
%the Syncom21 (this can be modified)
%PERMUTATION TEST
...
max_obs_biomass_21 = mean(table2array(Data_Evol_21(:, (end - nb_rep_21 + 1):end)), 2); 
max_obs_biomass_21 = [table(name), array2table(max_obs_biomass_21), Data_Evol_21(:, (end - nb_rep_21 + 1):end)];
max_obs_biomass_20 = mean(table2array(Data_Evol_20(:,(end - nb_rep_20 + 1):end)), 2); 
max_obs_biomass_20 = [table(name), array2table(max_obs_biomass_20), Data_Evol_20(:, (end - nb_rep_20 + 1):end)];
...
xvalues = categorical(inv_names.Name);
X = reordercats(xvalues, inv_names.Name);
yvalues_21 = max_obs_biomass_21.max_obs_biomass_21./(single_cell_mass.assumedCellDwInFgC*1e-15);
yvalues_20 = max_obs_biomass_20.max_obs_biomass_20./(single_cell_mass.assumedCellDwInFgC*1e-15);
...
for i = 3:(3 + nb_rep_21 - 1)
    scatter(flip(log10(table2array(max_obs_biomass_21(:,i)./(single_cell_mass.assumedCellDwInFgC*1e-15)))),1:S, 50, '.', 'r')
end
%Automatized this
for i = 3:(3 + nb_rep_20 - 1)
    scatter(flip(log10(table2array(max_obs_biomass_20(:,i)./(single_cell_mass.assumedCellDwInFgC*1e-15)))),1:S, 50, '.', 'b')
end
...
print(figH,FigName, '-dpdf', '-painters');
mat_21 = table2array(max_obs_biomass_21(:,3:(3 + nb_rep_21 - 1))./(single_cell_mass.assumedCellDwInFgC*1e-15));
mat_20 = table2array(max_obs_biomass_20(:,3:(3 + nb_rep_20 - 1))./(single_cell_mass.assumedCellDwInFgC*1e-15));
...
ind_syncom_test=[combined.Name table(ind_syncom_test) table(FDR) table(hvals) table(pvals)];
writetable(ind_syncom_test, 'Data/SynCom21-SynCom20-mattest.csv');

% take the difference of the means as fraction of the SynCom20 biomass

diff_vector = max_obs_biomass_21.max_obs_biomass_21 - max_obs_biomass_20.max_obs_biomass_20;
diff_fraction = diff_vector./max_obs_biomass_21.max_obs_biomass_21;
