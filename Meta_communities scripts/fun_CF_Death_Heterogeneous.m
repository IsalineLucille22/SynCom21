function dz_vect = fun_CF_Death_Heterogeneous(t, z, Colonization_Mat, kappa, CrossFeed_Mat, Mat_kappa_3, Resource_Matrix, Threshold_CF, Threshold_death, Death_Mat, death_rate, S, Lag_time_Cons, Lag_time_Pred, nb_Res, Hill_CF, Hill_Pred)
z(z<0) = 0;
...
T(isnan(T)) = 0;
k = Hill_Pred;
T_death = (1./(1 + (Threshold_death./z(:,1)).^k));%(1./(1 + (Threshold_death(1)./z(:,1)).^k));%1./(1 + (Threshold_death(1)./(max(z(:,3) - sum(r),0))).^k));%
...
dx_vect =  z(:,1).*sum(kappa(:,2).*((t > Lag_time_Cons).*Resource_Matrix).*(r'./(r' + K_S_res)),2)...
    ...
    + Colonization_Mat*z(:,1);%z(:,1).*(T_death.*death_rate);%(t > 168).* %Remove 0 multiplication %6.2855e-04 %Modify with time
