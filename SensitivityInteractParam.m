...
Name_file = 'Death_CF_Model_New_Mu_max_v21';%'Death_CF_Model_V17';%
...
cov_mat = cov(CrossFeed_Mat);
std_mat = 0.05*mean(Threshold);%mean(std(Lag_time_Cons))*ones(S, nb_Res);%repmat(std(Lag_time_Cons), S, 1);
for n = 1:nb_iter
    
    Threshold_temp = SensitivityChanges(Threshold, std_mat);
    ...
    % mean_y_0 = mean_y_0*sum_biomass/sum(mean_y_0);
    ...
    Threshold_temp = Threshold_temp(S_sim);
